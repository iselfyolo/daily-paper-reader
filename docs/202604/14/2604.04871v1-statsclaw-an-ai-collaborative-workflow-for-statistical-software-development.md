...
## 摘要
将统计方法转化为可靠的软件是定量研究中长期存在的瓶颈。现有的 AI 代码生成工具虽然产出迅速，但无法保证实现的忠实度，而这正是统计软件的关键要求。我们介绍了 StatsClaw，这是一种基于 Claude Code 的多智能体架构，它在代码生成与验证之间强制执行信息屏障。规划智能体（planning agent）为实现、模拟和测试生成独立的规范，并将其分发给彼此无法看到指令的独立智能体：构建者在不知道真实参数（ground-truth parameters）的情况下进行实现，模拟者在不知道算法的情况下生成数据，而测试者则使用确定性标准进行验证。我们描述了该方法，在一个 probit 估计包上进行了端到端的演示，并在作者自己的三个 R 和 Python 包应用中对其进行了评估。结果表明，结构化的 AI 辅助工作流可以承担软件生命周期中的工程开销，同时保留研究人员对每一项实质性方法决策的控制权。
