...
title_zh: SKILLFOUNDRY：从异构科学资源中构建自演化智能体技能库
...
tags: ["query:ds-gen"]
score: 6.0
evidence: 将科学资源转化为经过验证的智能体技能的框架
tldr: SkillFoundry是一个旨在解决科学领域知识碎片化问题的自我演化框架。它通过从API、论文和代码库等异构资源中挖掘程序性知识，将其转化为包含执行步骤和验证测试的可重用智能体技能包。该框架利用领域知识树进行系统化提取与闭环验证，显著提升了科学智能体在MoSciBench及基因组学等复杂任务中的表现，为构建高性能科学智能体奠定了基础。
selection_source: fresh_fetch
motivation: 科学领域存在大量异构的程序性知识，但智能体难以直接将其转化为可操作的技能，限制了其在复杂科学任务中的效能。
method: 提出SkillFoundry框架，通过构建领域知识树、提取操作契约并编译为可执行技能包，结合闭环验证实现技能库的自我演化。
result: "挖掘出的技能中71.1%为新增技能，且在MoSciBench及基因组学任务中显著提升了智能体的性能。"
conclusion: 自动挖掘的技能库能有效扩展智能体能力边界，为开发具备自我进化能力的科学智能体提供了实用且高效的方案。
---

## 摘要
现代科学生态系统在代码仓库、API、脚本、笔记本、文档、数据库和论文中蕴含着丰富的过程性知识，然而，这些知识中的大部分仍分散在异构的人工制品中，智能体无法直接将其转化为可操作的能力。丰富的科学诀窍与可用的智能体能力之间的这种差距，是构建高效科学智能体的关键瓶颈。我们提出了 SkillFoundry，这是一个自演化框架，可将此类资源转换为经过验证的智能体技能——即编码了任务范围、输入输出、执行步骤、环境假设、溯源信息和测试的可重用软件包。SkillFoundry 将目标领域组织为领域知识树，从高价值分支中挖掘资源，提取操作契约，将其编译为可执行的技能包，然后通过闭环验证过程迭代地扩展、修复、合并或剪枝生成的库。SkillFoundry 生成了一个具有显著创新性且内部有效的技能库，其中 71.1% 的挖掘技能与 SkillHub 和 SkillSMP 等现有技能库不同。我们证明，这些挖掘出的技能提高了编程智能体在六个 MoSciBench 数据集中的五个上的表现。我们进一步展示了 SkillFoundry 可以根据具体科学目标的需要设计新的特定任务技能，并且生成的技能显著提升了在两项具有挑战性的基因组学任务（细胞类型注释和 scDRS 工作流）上的性能。综上所述，这些结果表明，自动挖掘的技能提高了智能体在基准测试和特定领域任务上的表现，扩展了手工构建技能库之外的覆盖范围，并为构建更强大的科学智能体提供了实践基础。
