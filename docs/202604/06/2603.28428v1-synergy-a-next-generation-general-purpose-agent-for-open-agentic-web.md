...
score: 7.0
evidence: 去中心化数字生态系统，支持智能体发现与任务委派
...
## 摘要
AI 智能体的能力和规模正在迅速增长：它们现在能够编写代码、跨平台操作计算机、管理云基础设施并做出购买决策；与此同时，OpenClaw 等开源框架正将个人智能体普及给数百万用户，具身智能体也正广泛应用于智能手机、车辆和机器人。随着互联网准备承载数十亿个此类实体，它正在向我们所谓的“开放智能体网络”（Open Agentic Web）转型。这是一个去中心化的数字生态系统，来自不同用户、组织和运行时的智能体可以相互发现、协商任务边界，并在开放的技术和社交界面上实现大规模的任务委托。然而，目前大多数智能体仍是孤立的工具或封闭生态系统内的编排者，而非开放网络中具有社交集成能力的参与者。我们认为，下一代智能体必须成为“智能体公民”（Agentic Citizens），其定义包含三个核心要求：智能体网络原生协作，即参与开放协作网络而非仅限于封闭的内部编排；智能体身份与人格，即作为社交实体的连续性而非可重置的函数调用；以及终身演化，即随时间推移在任务表现、沟通和协作方面持续进步。我们提出了 Synergy，这是一种通用的智能体架构和运行时框架，旨在为开放智能体网络构建持久、协作且不断演化的智能体。Synergy 将协作建立在会话原生编排、基于仓库的工作空间和社交沟通之上；将身份建立在类型化记忆、笔记、议程、技能和持久社交关系之上；并将演化建立在一种以经验为中心的学习机制之上，该机制在推理阶段能主动召回获得奖励的轨迹。
