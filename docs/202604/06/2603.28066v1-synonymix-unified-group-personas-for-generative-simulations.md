...
## 摘要
生成式智能体模拟通常在两个尺度上运行：用于角色交互的个体画像，以及用于集体行为分析和干预测试的人口模型。我们提出了第三个尺度：中观层面模拟（meso-level simulation）——即与保留了丰富个体经验基础的群体级表征进行交互。为此，我们提出了 Synonymix，这是一个通过基于图的抽象与合并，从多个生活故事画像中构建“统一图”（unigraph）的流水线，从而产生一个可查询的集体表征，该表征可用于意义构建的探索或采样以生成合成画像。通过在综合社会调查（General Social Survey）项目上评估合成智能体，我们证明了其在人口统计基准之外的行为信号保留能力（p<0.001, r=0.59），并具有可证明的隐私保证（最大来源贡献率 <13%）。我们邀请各界讨论中观层面模拟所开启的交互模态，以及“高保真”画像是否能够真正捕捉到生活经验的质感。
