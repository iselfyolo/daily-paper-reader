...
score: 6.0
evidence: 用于交互式文档生成的多智能体流水线，帮助读者理解复杂概念
tldr: ViviDoc是首个系统性解决交互式文档生成的框架，旨在降低创作此类文档所需的高昂专业知识和开发成本。它采用多智能体协作流水线（规划、样式、执行、评估），并引入三层人类控制机制（结构化规范、样式调色板、对话式编辑）以增强生成过程的可控性。通过构建ViviBench基准测试和自动化评估框架，实验证明ViviDoc在内容丰富度和交互质量上均优于现有方法，显著提升了创作效率和用户满意度。
selection_source: fresh_fetch
motivation: 交互式文档创作成本高且现有LLM生成结果难以控制，亟需一种能平衡自动化与人类干预的生成系统。
method: 提出包含四个智能体的协作流水线，并结合SRTC交互规范、样式调色板及对话式编辑实现多层次的人机协作。
result: 在包含101个主题的ViviBench基准测试中，ViviDoc在内容丰富度和交互质量上均取得领先，且自动化评估与人类评分高度一致。
conclusion: ViviDoc通过人机协作有效降低了交互式文档的创作门槛，为复杂内容的动态呈现提供了高效且可控的解决方案。
---
