...
method: 提出 CLIPPER 框架，利用单样本上下文学习探测模型响应，通过模拟原始数据集的困惑度分布来筛选核心子集。
...
## 摘要
将世界知识注入预训练多模态大语言模型（MLLMs）对于特定领域应用至关重要。特定任务微调通过使 MLLMs 适配高质量领域内数据来实现这一目标，但随着数据集规模的增长，面临着可扩展性挑战，需要在性能和计算开销之间进行权衡。现有的数据选择方法依赖于额外的评分模型或启发式聚类，未能同时兼顾数据重要性和多样性。此外，这两种方法都忽略了训练样本之间的相互作用。为解决这些局限性，我们提出了 CLIPPER，这是一种无需训练的数据选择流水线，它分离了参数知识与世界知识，并利用上下文学习来探测模型对不同“示例-查询”组合的响应。CLIPPER 识别出能够反映原始数据集困惑度分布的核心集，在保持多样性的同时保留了关键样本。在两个 MLLMs 和三个数据集上的实验表明，CLIPPER 以显著更低的成本达到了全量微调的性能：Qwen2.5-VL-7B 在 VRSBench 上实现了 47% 的数据效率，而 Llama-3.2-11B-Vision-Instruct 将 ScienceQA 的训练时间缩短了 37%。
