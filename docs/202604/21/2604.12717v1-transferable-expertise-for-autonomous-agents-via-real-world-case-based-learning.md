...
## 摘要
基于大语言模型（LLM）的自主智能体在通用推理任务上表现良好，但在复杂的真实世界场景中，仍难以可靠地利用任务结构、关键约束和先验经验。我们提出了一种案例学习框架，将过去任务的经验转化为可重用的知识资产，使智能体能够将先前的案例经验迁移到新任务中，并进行更结构化的分析。与主要基于预训练知识或静态提示的方法不同，我们的框架强调从真实案例中提取并重用与任务相关的知识、分析性提示和操作技能。我们在包含六个复杂任务类别的统一基准上评估了该方法，并将其与零样本（Zero-Shot）、少样本（Few-Shot）、清单提示（Checklist Prompt）和规则记忆（Rule Memory）基线进行了比较。结果表明，我们的方法在所有任务中均表现出持续强劲的性能，在每种情况下都达到或超过了最佳基线，在更复杂的任务上提升尤为明显。进一步分析表明，案例学习的优势随任务复杂度的增加而增加，且一个智能体获取的实践知识可以被其他智能体重用。这些发现表明，案例学习为构建面向真实世界工作的专业智能体提供了一条充满前景的路径。
