...
tldr: 针对属性值提取（AVE）等任务中自回归解码速度慢的问题，本文提出超并行解码（HPD）算法。该方法利用输出序列间的独立性，通过位置ID操作实现乱序并行生成，并支持多文档堆叠。实验证明，HPD在保证质量的前提下，将推理速度和成本优化了高达13.8倍，具有极高的工业应用价值。
selection_source: fresh_fetch
motivation: 传统的自回归解码在处理具有多个独立输出序列的任务时效率低下，无法充分利用计算资源的并行性。
method: 提出Hyper-Parallel Decoding (HPD)，通过操纵位置ID实现跨批次的共享内存与计算，支持在单个Prompt中并行生成多个属性值及堆叠多个文档。
result: 在AVE任务上，该方法在不损失输出质量的情况下，将推理成本和总推理时间降低了高达13.8倍。
conclusion: HPD打破了自回归链的限制，为具有独立输出结构的生成任务提供了一种高效且通用的加速方案。
---

## 摘要
某些文本生成任务，如属性值提取（AVE），需要从同一文档上下文中解码多个独立的序列。虽然标准的自回归解码由于其顺序性而速度较慢，但输出序列之间的独立性为并行化提供了机会。我们提出了超并行解码（Hyper-Parallel Decoding, HPD），这是一种新型解码算法，通过利用跨批次的共享内存和计算来加速离线解码。HPD 通过位置 ID 操作实现乱序 Token 生成，显著提高了效率。在 AVE 上的实验表明，属性-值对是条件独立的，这使我们能够在每个提示词（prompt）内并行生成值。通过在单个提示词中进一步堆叠多个文档，我们可以在每个提示词中并行解码多达 96 个 Token。HPD 适用于所有大语言模型（LLM），在不损失输出质量的情况下，将推理成本和总推理时间降低了高达 13.8 倍，有望在工业级 AVE 任务中节省数十万美元。尽管 HPD 是为属性提取设计的，但它没有针对 AVE 领域的独特假设，理论上可以应用于其他具有独立输出结构的场景。
