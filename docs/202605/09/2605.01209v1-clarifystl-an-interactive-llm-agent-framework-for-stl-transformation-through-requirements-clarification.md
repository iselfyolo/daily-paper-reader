...
tldr: ClarifySTL是一个交互式大语言模型代理框架，旨在解决将自然语言需求转换为信号时序逻辑（STL）时存在的模糊和歧义问题。该框架通过自动检测需求中的不确定信息并生成针对性的澄清询问，引导用户补充细节并消除歧义，从而确保生成的STL公式准确反映用户意图。实验表明，该方法在多个基准测试中表现出色，有效提升了转换的准确性。
selection_source: fresh_fetch
motivation: 现有的自然语言转STL方法在处理实际应用中常见的模糊或歧义需求时表现不佳，难以准确捕捉用户意图。
method: 提出一种交互式框架，通过检测模糊表达、生成澄清查询并根据用户反馈更新需求，最后利用LLM将澄清后的需求转换为STL。
result: 在DeepSTL、STL-DivEn及新提出的AmbiEval基准测试上的实验结果证明了ClarifySTL在检测模糊性、生成查询及转换准确性方面的有效性。
conclusion: ClarifySTL通过交互式澄清机制显著增强了LLM处理复杂需求的能力，为CPS系统的形式化规格说明提供了更可靠的自动化工具。
---

## 摘要
信号时序逻辑（STL）是一种用于规约信息物理系统（CPS）实时行为的正式语言。将自然语言需求自动转换为 STL 规范已受到越来越多的关注。近期利用大语言模型（LLM）的研究已展现出显著的性能，但在实践中，部分自然语言需求包含模糊或歧义的信息，这对于 LLM 而言仍具挑战性。为应对这些挑战，我们提出了 ClarifySTL，这是一种通过需求澄清来增强 STL 转换的交互式 LLM 智能体框架。ClarifySTL 首先检测需求中指示信息不全的模糊表达。若检测到模糊性，它会生成有针对性的澄清查询，引导用户补充需求，直到提供所有必要细节。随后，若 ClarifySTL 检测到歧义，它会制定针对性的歧义澄清查询，并根据用户反馈更新需求，直到所有歧义得到解决。最后，经过模糊性和歧义澄清的需求将使用 LLM 转换为 STL 规范。该交互式框架确保生成的 STL 公式能够忠实地捕捉用户意图，同时减轻用户的负担。我们在代表性基准测试 DeepSTL 和 STL-DivEn，以及我们新引入的 AmbiEval 基准测试上评估了 ClarifySTL。AmbiEval 专门设计用于评估智能体在处理模糊性和歧义性方面的性能，包括检测和查询生成。实验结果表明 ClarifySTL 是有效的。
