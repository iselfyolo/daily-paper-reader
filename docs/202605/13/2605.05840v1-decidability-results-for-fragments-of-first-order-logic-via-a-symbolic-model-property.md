...
title_zh: 通过符号模型性质获得的一阶逻辑片段的可判定性结果
authors: "Neta Elad, Sharon Shoham"
date: 2026-05-07
pdf: "https://arxiv.org/pdf/2605.05840v1"
tags: ["query:q6"]
score: 7.0
evidence: 形式逻辑片段与符号模型属性
...
## 摘要
最近，符号结构被提出作为潜在无限一阶结构的有限表示，其中线性整数算术（Linear Integer Arithmetic）项和公式定义了结构的论域和解释。我们将符号结构推广到可以使用任何允许标准模型的基理论。符号结构引出了一种符号模型性质，如果一个一阶逻辑片段中的每个可满足公式都具有符号模型，则该片段满足该性质。符号模型性质意味着可判定性，因为符号结构的模型检测问题是可判定的。我们利用符号模型性质证明了扩展分层公式（stratified formulas）片段的若干片段的可判定性，在特定限制下，通过允许一种类型（sort）具有自循环函数来放宽量词交替约束。为了建立这些片段的符号模型性质，我们从任意模型中为公式构造了一个符号模型。该构造及其正确性以通用方式进行了证明，可以实例化到其他具有类似限制的片段。
