...
tags: ["query:q9"]
score: 6.0
evidence: 融合抽象语法树（AST）表示以实现结构感知
tldr: 针对软件文档与代码逻辑不一致导致的维护难题，本文提出 DocSync 智能体工作流。该方法结合抽象语法树（AST）与检索增强生成（RAG）提供结构化上下文，并引入基于 Reflexion 范式的批评引导精炼循环。实验表明，即使使用轻量化模型，DocSync 在语义一致性和忠实度上仍显著优于传统基准，有效降低了文档维护的技术债。
selection_source: fresh_fetch
motivation: 软件文档常随代码演进而失效，而现有工具难以处理语义不一致或在更新时产生幻觉。
method: 提出一种融合 AST 表示、RAG 检索及批评引导自我修正循环的智能体工作流。
result: 在代码到文本维护任务中，DocSync 的表现显著优于 CodeT5 等基准模型，自动化评分从 1.91 提升至 3.44。
conclusion: 结合结构化检索与智能体精炼循环是自主缓解文档技术债、提升文档准确性的有效途径。
---

## 摘要
随着代码库的演进，软件文档经常与可执行逻辑脱节，从而产生技术债务，降低可维护性并导致下游 API 的误用。虽然静态分析工具可以检测文档的缺失，但它们无法评估其语义一致性。相反，标准大语言模型（LLMs）虽然提供了生成的灵活性，但在缺乏对底层代码深度结构感知的情况下更新文档时，经常会出现幻觉。为了填补这一空白，我们提出了 DocSync，这是一种将文档维护框架化为基于结构的迭代生成任务的智能体工作流。DocSync 通过融合抽象语法树（AST）表示和检索增强生成（RAG）来提供依赖感知的上下文，从而桥接语法变化与自然语言描述。此外，为了确保事实一致性，我们引入了一个基于 Reflexion 范式的评论者引导优化循环，允许模型根据源代码对候选更新进行自我修正。我们在代理代码到文本维护任务上，对 DocSync 的一种资源受限实现（使用 LoRA 适配的小语言模型）进行了实证评估。研究结果表明，这种感知 AST 的智能体方法在语义对齐、摘要行忠实度以及自动评测偏好方面显著优于标准的编码器-解码器基准模型（例如，自动评测得分达到 3.44/5.0，而 CodeT5-base 仅为 1.91）。至关重要的是，迭代评论者循环在不需要增加参数量的情况下，实现了语义正确性的显著提升。这些结果提供了强有力的证据，表明将结构化检索与智能体优化相结合是自主缓解文档债务的一个极具前景的方向。
