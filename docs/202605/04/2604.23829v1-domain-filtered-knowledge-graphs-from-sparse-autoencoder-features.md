...
## 摘要
稀疏自编码器 (SAE) 从语言模型中提取了数百万个可解释特征，但扁平化的特征清单本身并不十分有用。领域概念往往与通用且基础薄弱的特征混杂在一起，相关概念散布在多个单元中，且无法理解特征之间的关系。为了解决这一问题，我们首先利用对比激活和多阶段过滤过程，从庞大的 SAE 特征库中构建了一个严格的特定领域概念全集。接着，我们在过滤后的集合上构建了两个对齐的图视图：一个是用于语料库级概念结构的共现图（按多个粒度级别组织），另一个是基于转码器 (transcoder) 的机制图（通过稀疏潜路径连接源层和目标层特征）。随后，自动化的边标注将这些图视图转化为可读的知识图谱，而非无标签的布局。在一项关于生物学教科书的案例研究中，这些图谱恢复了连贯的章节和子章节级结构，揭示了连接相邻主题的概念，并将包含数千个特征的杂乱句子级活动转化为简洁、可读的视图，展示了模型的局部活动。总之，这项工作将扁平的 SAE 特征库重构为内部知识图谱，将特征级可解释性转化为模型知识的全局图谱，并实现了对推理忠实度的审计。
