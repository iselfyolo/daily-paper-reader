...
tldr: 本研究揭示了“推理陷阱”现象，即多智能体辩论等闭环推理系统在保持答案准确性的同时，会因信息论中的数据处理不等式（DPI）导致推理质量退化。作者提出了SFS评估指标和EGSR推理方法，并通过理论证明和实验显示，闭环系统中的证据关联度随步骤增加而下降，而引入外部证据的EGSR能有效恢复推理的忠实度。
selection_source: fresh_fetch
motivation: 旨在解决多智能体辩论等闭环推理系统中，模型输出虽能维持准确率但推理过程逐渐偏离证据、质量下降的问题。
method: 提出了基于原子声明验证的SFS指标，并开发了以证据为导向的苏格拉底式推理（EGSR）框架，同时利用信息论DPI定理证明了闭环推理的局限性。
result: "实验表明传统辩论模式下推理忠实度大幅下降（SFS最高下降98%），而EGSR能恢复98%的忠实度并打破信息衰减限制。"
conclusion: 任何遵循马尔可夫链结构的闭环推理协议在预期上都会受限于信息论边界，必须引入外部证据反馈才能打破推理退化的陷阱。
---

## 摘要
当同一个语言模型的多个副本被提示进行辩论时，它们会产生对单一观点的多样化措辞，而非多样化的观点。多智能体辩论（MAD），以及更广泛的封闭系统推理（即智能体迭代地转换彼此的输出），往往在保持答案准确性的同时，降低了这些答案背后的推理质量。我们将多智能体的情况称为“辩论陷阱”（Debate Trap），将更广泛的现象称为“推理陷阱”（Reasoning Trap），并提供了一个关于基于证据的推理失败的程序化理论。该框架包含三个部分：(i) SFS（支持忠实度得分），一种在断言层面根据提供的证据验证分解后的原子断言的指标（分解器不变排名：Spearman rho=1.0）；(ii) EGSR（基于证据的苏格拉底式推理），用基于证据的询问取代对抗性论证；(iii) 定理 1（DPI 界限）：在标准 MAD 下，链条 E -> O^0 -> O^1 -> ... 是马尔可夫链，且数据处理不等式（DPI）意味着 E[I(E;O^{t+1})] <= E[I(E;O^t)]。三个伴随结果——开放系统恢复（定理 2）、EGSR 累积（引理 2）和投票聚合下限（命题 1）——根据多步 LLM 推理与 E 的信息论关系对其进行了划分。在 SciFact（300 个断言）和 FEVER（1,000 个断言）的 16 种条件下，DebateCV (C13) 保留了 88% 的基准准确率，而 SFS 下降了 43%；多数投票 MAD (C15) 将 SFS 降低至基准的 1.7% (p < 10^{-6}, d = -0.96)；EGSR 恢复了 98%。一项 R6 队列研究（韩语 n=10x30 FEVER；英语 n=3x200 SciFact）发现，跨语言和领域的评分者间 Fleiss kappa <= +0.018，且评分者内存在 0.8-1.4 的 Likert 量表偏移——忠实度指标所校准的人类一致性本身并不稳定。我们提出了一个可证伪的猜想：任何保持定理 1 马尔可夫结构的封闭系统推理协议，在期望上都受制于相同的 DPI 界限。
