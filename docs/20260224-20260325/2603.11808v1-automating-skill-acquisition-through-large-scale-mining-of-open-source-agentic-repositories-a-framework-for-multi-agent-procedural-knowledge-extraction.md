...
tags: ["query:edu-llm"]
score: 8.0
evidence: 为智能体大模型提取教育能力
tldr: 本研究针对大语言模型在专业程序性知识方面的不足，提出了一种从开源代码库中自动挖掘并提取智能体技能的系统化框架。通过对TheoremExplainAgent等系统的可视化与教育能力进行结构化分析与语义识别，并将其转化为标准化的SKILL.md格式，实现了无需重训即可扩展智能体能力的目标。该方法在提升知识传递效率的同时，为构建模块化、具备专业技能的智能体提供了高效且可扩展的路径。
selection_source: fresh_fetch
motivation: 解决通用大语言模型在处理复杂自动化工作流时，因缺乏专业程序性知识而导致的功能受限问题。
method: 提出一种结合仓库结构分析、密集检索语义识别及标准化格式转换的自动化技能挖掘框架。
result: "实验证明该框架能有效提取高质量技能，使智能体生成的教育内容在知识传递效率上提升了40%。"
conclusion: 通过大规模挖掘开源代码库，可以实现智能体专业技能的低成本、可扩展获取，显著增强其应用价值。
---
