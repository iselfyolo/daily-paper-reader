...
## 摘要
交互式文章通过探索帮助读者理解复杂的概念，但其创作成本依然高昂，需要同时具备领域专业知识和 Web 开发技能。近期基于大语言模型（LLM）的智能体可以自动化内容创作，但直接应用这些智能体往往会产生不可控且难以验证的输出。我们提出了 ViviDoc，这是一个通过单一主题输入即可生成交互式教育文档的人机协作系统。ViviDoc 引入了一个多智能体流水线（规划器、执行器、评估器）以及文档规范（DocSpec），这是一种人类可读的中间表示，将每个交互式可视化分解为状态（State）、渲染（Render）、转换（Transition）和约束（Constraint）组件。DocSpec 使教育工作者能够在生成代码之前审查并完善生成计划，从而弥合了教学意图与可执行输出之间的鸿沟。专家评估和用户研究表明，ViviDoc 的表现显著优于朴素的智能体生成方式，并提供了直观的编辑体验。我们的项目主页位于 https://vividoc-homepage.vercel.app/。
