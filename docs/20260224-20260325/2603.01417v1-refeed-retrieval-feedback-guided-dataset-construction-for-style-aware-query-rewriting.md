...
tags: ["query:ds-gen"]
score: 9.0
evidence: 利用大语言模型的检索反馈驱动数据集生成框架
tldr: 针对检索系统中用户查询与领域文档在风格和语义上存在差异的问题，本文提出ReFeed框架。该框架利用检索反馈自动识别失败案例，通过大语言模型将查询重写为符合目标文档风格的形式，并经重新检索验证效果。通过构建高质量的（原始，重写）查询对数据集，训练出的重写模型能显著提升检索系统在特定领域中的适应性和准确性。
selection_source: fresh_fetch
motivation: 现有的查询重写方法往往忽略了目标文档的领域特定风格（如语气和结构），导致检索效果受限。
method: 提出一种由检索反馈驱动的自动化框架，结合LLM生成风格对齐的重写查询，并通过闭环验证构建训练数据集。
result: 成功构建了包含风格感知查询对的语料库，并证明了基于此训练的重写模型能有效提升检索性能。
conclusion: 强调了反馈循环和文档风格对齐在增强RAG系统领域适应性方面的重要性，为以数据为中心的检索研究提供了新方向。
---
