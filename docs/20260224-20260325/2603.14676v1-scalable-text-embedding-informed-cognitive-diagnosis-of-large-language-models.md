...
tags: ["query:edu-llm"]
score: 8.0
evidence: 将认知诊断模型应用于LLM评估
tldr: 本研究针对大语言模型（LLM）评估中粗粒度指标无法揭示底层推理能力的问题，提出了一种可扩展的认知诊断模型（CDM）框架。该方法通过联合估计LLM的掌握程度分布和题目-属性Q矩阵，解决了高维潜在属性和大规模题库带来的计算挑战。通过引入文本嵌入先验知识，减少了对人工标注的依赖并提高了估计稳定性。在MATH Level 5基准上的应用证明了该方法在揭示LLM细粒度能力方面的实用性。
selection_source: fresh_fetch
motivation: 现有的LLM评估主要依赖粗略的汇总指标，难以深入分析模型在特定认知维度上的优劣势。
method: 提出一种结合文本嵌入先验的可扩展随机逼近算法，用于在高维属性空间下联合估计LLM的能力画像和Q矩阵。
result: 模拟实验验证了参数恢复的准确性，在MATH数据集上的应用成功揭示了不同LLM在细粒度知识点上的掌握差异。
conclusion: 该研究为大规模LLM评估提供了一种高效、可解释且减少人工干预的认知诊断新范式。
---
