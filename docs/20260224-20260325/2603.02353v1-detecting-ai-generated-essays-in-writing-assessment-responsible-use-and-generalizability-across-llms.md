...
title_zh: 写作评估中 AI 生成文章的检测：负责任的使用及跨大语言模型（LLM）的泛化性
authors: Jiangang Hao
date: 2026-03-02
pdf: "https://arxiv.org/pdf/2603.02353v1"
tags: ["query:edu-llm"]
score: 9.0
evidence: 检测AI生成的作文及在教育中的负责任使用
...
conclusion: 研究强调了在教育评估中负责任地应用检测技术的重要性，并为开发更具鲁棒性、能应对模型演进的检测工具提供了实践建议。
---

## 摘要
写作是一项基础的素养技能，它支撑着有效的沟通，培养批判性思维，促进跨学科学习，并使个人能够组织和表达复杂的思想。因此，写作评估在评价语言熟练度、沟通有效性和分析性推理方面发挥着至关重要的作用。大语言模型（LLM）的飞速发展使得生成连贯、高质量的文章变得日益容易，这引发了人们对学生提交作品真实性的极大担忧。本章首先概述了当前 AI 生成和 AI 辅助文章检测器的现状，并提出了负责任使用的指南。随后，基于针对公开 GRE 写作题目生成的文章，本章通过实证分析评估了在某一 LLM 生成的文章上训练的检测器在识别其他 LLM 生成的文章时的泛化能力。这些研究结果为实际应用中检测器的开发和重新训练提供了指导。
