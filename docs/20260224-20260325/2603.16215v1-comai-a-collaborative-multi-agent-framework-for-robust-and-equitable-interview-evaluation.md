...
score: 9.0
evidence: 用于面试评估和评分的协作式多智能体框架
tldr: CoMAI是一个多智能体协作框架，旨在解决AI面试评估中的鲁棒性与公平性问题。该框架采用模块化任务分解架构，通过有限状态机协调提问、安全、评分和总结四个专业智能体。它能有效防御提示词注入，支持自适应难度调整和结构化评分，显著提升了面试评估的准确性、安全性和候选人满意度。
selection_source: fresh_fetch
motivation: 针对现有单体LLM面试系统在鲁棒性、公平性及安全性方面的不足，提出更可靠的评估方案。
method: 采用基于有限状态机协调的多智能体架构，将面试任务分解为提问、安全防御、评分和总结四个专业模块。
result: "实验显示CoMAI实现了90.47%的准确率和83.33%的召回率，且候选人满意度达到84.41%。"
conclusion: CoMAI为AI驱动的面试评估提供了一个鲁棒、公平且可解释的高效范式。
---
