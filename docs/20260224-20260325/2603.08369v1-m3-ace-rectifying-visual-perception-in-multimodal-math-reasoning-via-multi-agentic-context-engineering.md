...
motivation: 研究发现多模态数学推理的失败主要源于视觉证据提取的错误或缺失，且模型往往对初始错误感知过度自信，现有方法难以纠正。
method: 提出一种多智能体上下文工程框架，通过解耦感知与推理，利用多个智能体协作维护共享证据列表，并配合总结与精炼工具进行迭代修正。
result: 该方法在MathVision基准测试上取得了89.1的SOTA成绩，并在MathVista和MathVerse等多个数据集上实现了显著的性能提升。
conclusion: 实验证明，以感知为中心的多智能体协作能有效纠正视觉感知偏差，是提升多模态数学推理系统准确性的有效途径。
---
