...
tags: ["query:ds-gen"]
score: 8.0
evidence: 基于智能体的自动化数据准备系统和工作流
tldr: 本文从以数据为中心的视角探讨了下一代大语言模型（LLM）的训练。针对当前数据准备过程依赖临时脚本、缺乏自动化工作流以及训练中数据利用效率低等瓶颈，作者提出了两大研究方向：一是构建基于智能体的自动化数据准备系统，实现可扩展的数据管理；二是建立统一的数据-模型交互训练系统，通过动态选择和加权优化数据利用。该研究旨在提升LLM训练的效率与自适应性。
selection_source: fresh_fetch
motivation: 当前LLM训练数据准备过程繁琐且缺乏自动化，且训练时对海量数据的利用缺乏系统性的动态优化机制。
method: 提出构建基于智能体的自动化数据准备系统，并倡导建立支持动态数据选择、混合与重加权的统一数据-模型交互训练框架。
result: 该方案能够将数据科学家从重复的工程劳动中解放出来，并通过更智能的数据利用方式提升模型训练的效率和性能。
conclusion: 以数据为中心的自动化与动态交互系统是下一代LLM训练的关键，未来需重点攻克系统构建与数据优化策略等挑战。
---

## 摘要
大语言模型（LLMs）在广泛的任务和领域中展现出了卓越的性能，而数据在推动这些进展中发挥了核心作用。尽管取得了这些成功，但 LLM 训练所需的大规模数据集的准备和有效利用仍然是主要的瓶颈。在当前的实践中，LLM 训练数据通常是使用临时脚本构建的，目前仍缺乏成熟的、基于智能体（agent-based）的数据准备系统，这类系统能够自动构建稳健且可重用的数据工作流，从而将数据科学家从重复且易错的工程工作中解放出来。此外，数据集一旦收集完成，在训练过程中通常被整体消耗，缺乏数据选择、配比优化或重加权的系统性机制。为了解决这些局限性，我们提倡两个互补的研究方向。首先，我们建议构建一个稳健的、基于智能体的自动数据准备系统，支持自动化工作流构建和可扩展的数据管理。其次，我们主张建立一个统一的数据-模型交互训练系统，在该系统中，数据在整个训练过程中被动态地选择、混合和重加权，从而实现更高效、自适应且感知性能的数据利用。最后，我们讨论了剩余的挑战，并概述了未来研究和系统开发的有前景的方向。
