...
tldr: 针对多智能体系统在缺乏日志时难以追溯责任的问题，本文提出隐式执行追踪（IET）框架。该方法通过在生成过程中将特定密钥信号嵌入Token分布，使最终文本成为可自描述的执行轨迹。IET能在不依赖元数据的情况下，实现Token级的贡献归属和交互拓扑重建，在保持生成质量的同时，为多智能体系统提供了高精度的隐私保护审计手段。
selection_source: fresh_fetch
motivation: 多智能体系统生成的最终文本往往掩盖了底层的交互逻辑，导致在出现错误或有害输出时难以进行责任归属和审计。
method: 通过在生成阶段将智能体特定的密钥信号嵌入Token分布，并利用转换感知评分方法识别切换点以重建交互拓扑。
result: 实验结果显示，IET能够高精度地恢复智能体片段和协调结构，同时有效保留了原始文本的生成质量。
conclusion: IET为多智能体语言系统提供了一种无需元数据的归因方案，在保障隐私的前提下增强了系统的可解释性与问责制。
---
