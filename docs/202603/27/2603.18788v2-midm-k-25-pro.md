...
## 摘要
不断演进的大语言模型（LLM）格局要求具备超越简单文本生成的能力，优先考虑多步推理、长上下文理解和智能体工作流。这一转变对企业环境中的现有模型提出了挑战，特别是在韩语和特定领域场景下，单纯的规模扩展已不足够。我们推出了 Mi:dm K 2.5 Pro，这是一款拥有 32B 参数的旗舰级 LLM，旨在通过以推理为核心的优化来应对企业级的复杂性。我们的方法论通过以质量为中心的策划流水线构建了坚实的数据基础，该流水线利用抽象语法树（AST）分析处理代码，利用填空合成处理数学，并采用基于 LLM 的质量评估器。预训练阶段通过基于层预测器的深度扩展（DuS）和支持 128K token 上下文窗口的渐进式策略来扩展模型规模。后训练阶段引入了专门的多阶段流水线，包括推理有监督微调（Reasoning SFT）、模型合并和异步强化学习（RL），以培养解决复杂问题的能力。随后，“融合训练”（Fusion Training）将这些能力与对话流畅度、一致的回答风格以及可靠的工具使用进行重新平衡。评估结果表明，Mi:dm K 2.5 Pro 与全球和韩国国内领先的模型相比，具有极具竞争力的性能。此外，它在韩语特定基准测试中取得了最先进的结果，展示了深厚的语言和文化理解。最后，负责任 AI 评估验证了其抵御攻击的安全性，确保了部署时的安全概况，并在无害性和响应性之间取得了平衡。
