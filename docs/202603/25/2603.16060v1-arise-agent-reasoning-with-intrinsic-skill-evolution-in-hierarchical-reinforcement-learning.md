...
## 摘要
提升语言模型数学推理能力的主流范式依赖于具有可验证奖励的强化学习。然而，现有方法孤立地处理每个问题实例，未能利用训练过程中出现并积累的可重用策略。为此，我们提出了 ARISE（通过内在技能演化的智能体推理），这是一个分层强化学习框架，其中共享策略既在顶层管理技能，又在底层生成响应（分别称为技能管理器和执行器）。管理器通过专门的技能生成展开（在执行后对成功的解题轨迹进行结构化总结）来维护分层技能库，同时采用策略驱动的选择机制来检索相关技能，以指导未来的展开（在执行前）。分层奖励设计引导了推理能力与技能库质量的协同演化。在两个基础模型和涵盖竞赛数学及 Omni-MATH 的七个基准测试上的实验表明，ARISE 始终优于 GRPO 系列算法和记忆增强基线，在分布外任务上的提升尤为显著。消融研究证实，每个组件都对观察到的改进有所贡献，且技能库质量与推理性能在整个训练过程中同步提升。代码可在 https://github.com/Skylanding/ARISE 获取。
