...
## 摘要
多智能体大语言模型（LLM）系统（即由多个提示驱动的语言模型实例独立回答问题）正越来越多地被用于复杂的推理任务。然而，现有的量化其集体输出不确定性的方法依赖于浅层的投票统计，这丢弃了智能体推理中丰富的语义信息。我们引入了 DiscoUQ，这是一个提取并利用智能体间分歧结构的框架——包括语言属性（证据重叠、论证强度、分歧深度）和嵌入几何（聚类距离、离散度、凝聚力）——以产生校准良好的置信度估计。我们提出了三种复杂度递增的方法：DiscoUQ-LLM（对 LLM 提取的结构特征进行逻辑回归）、DiscoUQ-Embed（对嵌入几何进行逻辑回归）以及 DiscoUQ-Learn（结合所有特征的神经网络）。在四个不同的基准测试（StrategyQA、MMLU、TruthfulQA、ARC-Challenge）上，使用 Qwen3.5-27B 的 5 智能体系统进行评估，DiscoUQ-LLM 实现了 0.802 的平均 AUROC，优于最佳基准（LLM Aggregator，0.791），同时校准效果显著更好（ECE 为 0.036，而基准为 0.098）。所学习的特征在不同基准测试间具有泛化性，性能几乎没有下降，并在最需要的地方提供了最大的改进：即在简单投票计数失效的模糊“弱分歧”层级。
