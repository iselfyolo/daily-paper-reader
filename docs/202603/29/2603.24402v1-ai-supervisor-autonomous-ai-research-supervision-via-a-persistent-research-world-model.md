...
score: 8.0
evidence: 用于自主研究指导的多智能体编排
tldr: 针对现有自动化研究系统缺乏持久理解且呈线性流水线的局限，本文提出AutoProf框架。该框架采用多智能体协作模式，核心是基于知识图谱构建的“研究世界模型”，作为智能体间的共享内存。AutoProf实现了从文献综述、缺口发现到方法开发及论文撰写的全流程自动化。其主要贡献包括模块化缺口分析、自我修正的发现循环以及跨领域自我改进机制，并通过共识机制确保研究发现的准确性，为自主AI研究提供了系统化的监督方案。
selection_source: fresh_fetch
motivation: 现有自动化研究系统多为无状态的线性流水线，缺乏对研究领域的持久理解及智能体间的验证机制。
method: 提出AutoProf框架，利用知识图谱构建研究世界模型，通过多智能体协作实现结构化缺口发现、自我修正与改进循环。
result: 实现了从文献调研到论文撰写的全流程自主研究监督，支持跨领域机制搜索以迭代优化失败的研究组件。
conclusion: AutoProf通过持久化共享内存和共识验证机制，克服了传统系统的局限，提升了自主AI研究的深度与可靠性。
---
